...
#version v2.0 Uses a different density profile (steeper)
#             to represent a central cavity and a steeper outer profile
#             to transition to vacuum quicker
...
#
seed     = 42
R_max    = 4.2 * R_disk # maximum sampling radius
if (N_gas < 50000): R_bins = 100
elif (N_gas < 100000): R_bins = 200
elif (N_gas < 200000): R_bins = 400
elif (N_gas < 500000): R_bins   = 500
elif (N_gas < 1000000): R_bins   = 600
else : R_bins = 1000
R_cut    = 3.2 * R_disk
...
SigmaPrefactor = -(-3 + p) * (-3 + p) / gamma(1.0/(-3 + p))
print SigmaPrefactor

def GasSigmaAnalytic(R):
  return MassCorrection *  SofteningCorrection * TruncationCorrection *\
         SigmaPrefactor * M_disk/2/np.pi/R_disk**2 * (R_disk/R)**p  * np.exp(-(R/R_disk)**(3.0-p))

def GasSigma(R):
  if (not CentralCavity):
    if (not StarAccretion):
      return MassCorrection *  SofteningCorrection * TruncationCorrection *\
          SigmaPrefactor * M_disk/2/np.pi/R_disk**2 * R_disk**p * SplineProfile(R,dens_soft)**p \
          * np.exp(-(np.abs(R)/R_disk)**(2.0-p))
    else:
      return MassCorrection *  SofteningCorrection * TruncationCorrection *\
          SigmaPrefactor * M_disk/2/np.pi/R_disk**2 * R_disk**p * (SplineDerivative(R,dens_soft)* R * R)**p \
          * np.exp(-(np.abs(R)/R_disk)**(2.0-p))
  else:
    if (R < R_min):
      return SURFACE_DENSITY_FLOOR
    else:
      return MassCorrection *  SofteningCorrection * TruncationCorrection *\
          (SigmaPrefactor * M_disk/2/np.pi/R_disk**2 * R_disk**p * SplineProfile(R,dens_soft)**p * np.exp(-(np.abs(R)/R_disk)**(3.0-p))* np.exp(- R_cavity * R_cavity / R / R) + SURFACE_DENSITY_FLOOR)
...
first_cell_radius = MasstoRadius(M_disk/N_gas)

#we redo the radial bins based on the cumulative mass function
bin_concentration = 0.7
bin_break = 0.05
...
radial_bins0=  10**np.arange(np.log10((R_disk)*10**-3.5),np.log10(radial_bins1.min()),
                             (np.log10(radial_bins1.min())-np.log10((R_disk)*10**-3.5))/20)
print radial_bins0, radial_bins1
radial_bins=np.append(radial_bins0,
                      np.append(radial_bins1,radial_bins2))
#radial_bins=MasstoRadius(np.linspace(0.0,1.0,INTERPOL_BINS)*mass_bins_gas.max())

#radial_bins = np.append(radial_bins,np.linspace(R_max,1.1*R_max,20))
INTERPOL_BINS = radial_bins.shape[0]

mass_bins_gas=vecGasMass(radial_bins)
MasstoRadius=InterpolatedUnivariateSpline(mass_bins_gas, radial_bins,k=1)

#recalculate the surface density function for easier evaluation ##########
temp_radii = np.linspace((R_disk)*1e-4,R_disk*10,20000)
sigma_points = vecSigma(temp_radii)
vecSigma = interp1d(temp_radii,sigma_points,kind='linear')
...
#plt.loglog(temp_radii,vecSigma(temp_radii))
#plt.loglog(radial_bins,radial_bins*0+1e-7,'ro')
#plt.show()

#plt.loglog(temp_radii,vecGasMass(temp_radii))
#plt.loglog(radial_bins,radial_bins*0+1e-7,'ro')
#plt.loglog(radial_bins,radial_bins*0+M_disk,'--')
#plt.axis([radial_bins.min(),radial_bins.max(),M_disk*0.9,M_disk*1.1])
#plt.show()

print "\nIntegrating vertical structure..."
print "   Looping over ",INTERPOL_BINS," radial bins..."
...
for k in range(0,INTERPOL_BINS):
  ...
  if (k%50 ==0):
    print "...\n",
  if (k%100 ==0):
    print "...",'%4.1f' % (100.0*k/INTERPOL_BINS),"% completed"
  ...
  if not (SelfGravity): #no self-gravity
    ...
    density_interp=InterpolatedUnivariateSpline(vertical_bins,density, k=1)
  ...
  if (N_in_bin != 0):
    ...
    if(add_background) & (not load_mesh) & (radius < R_cut) & (radius > first_cell_radius) :

      first = MasstoZ((1.0-leftover_mass)*cumulative.max())
      first = max(np.abs(z_gas_at_R).max(),first)

      if (radius < 0.1 * R_disk): N_back_in_bin = np.int(0.1* N_in_bin)
      else:  N_back_in_bin = np.int(0.05* N_in_bin)



      if (N_back_in_bin > 0.1 * min_cells_per_bin):
        print radius, first_cell_radius, N_back_in_bin, N_in_bin

        #a dense envelope of cells surrounding the disk
        R_back_at_R = np.repeat(radius,N_back_in_bin)
        z_back_at_R = np.logspace(np.log10(first),np.log10(min(first*15.0,1.05*vertical_bins.max(),z_max)),N_back_in_bin)
        ...
        if (z_back_at_R.shape[0] > 10):
          on = (rd.random_sample(R_back_at_R.shape[0]) <= 0.4)
        ...
        if (k % 2 == 0): #only every other radial bin
          if (vertical_bins.max() < z_max):
            N_back_in_bin = N_back_in_bin/8 #reduce the number of sampled locations
            R_back_at_R = np.repeat(radius,N_back_in_bin)
...
#vecEffectiveGasSigma = InterpolatedUnivariateSpline(np.append(R_min,radius_per_bin),
#                                                    np.append(SURFACE_DENSITY_FLOOR,effective_gas_sigma),
#                                                    k=1)
vecEffectiveGasSigma = interp1d(np.append(np.append(0.0,radius_per_bin),R_cut),
                                np.append(np.append(SURFACE_DENSITY_FLOOR,effective_gas_sigma),DENSITY_CUTOFF),
                                kind='linear')

plot_effective_sigma=1
if (plot_effective_sigma):
  plt.loglog(np.linspace(R_min,R_cut,3000),vecSigmaAnalytic(np.linspace(R_min,R_cut,3000)))
  plt.loglog(np.linspace(R_min,R_cut,3000),vecSigma(np.linspace(R_min,R_cut,3000)))
  plt.loglog(np.linspace(R_min,R_cut,3000),vecEffectiveGasSigma(np.linspace(R_min,R_cut,3000)))
  plt.loglog(radius_per_bin,effective_gas_sigma,'r+')
  plt.show()

def GasMass_Integrand(R): return vecEffectiveGasSigma(R) * R * 2 * np.pi
def EffectiveGasMass(R): return quad(GasMass_Integrand,0.0,R,limit=100)[0]
vecEffectiveGasMass = np.vectorize(EffectiveGasMass)
print "MASS", quad(GasMass_Integrand,0.0,R_cut,limit=100)[0]
#temp_radii=np.linspace(1.0,R_cut,2000)
#plt.loglog(temp_radii,vecEffectiveGasMass(temp_radii))
#plt.loglog(temp_radii,vecGasMass(temp_radii))
#plt.loglog(radial_bins,radial_bins*0+M_disk,'--')
#plt.axis([temp_radii.min(),temp_radii.max(),M_disk*0.01,M_disk*1.02])
#plt.show()
...
vecRhoGasInPlane = InterpolatedUnivariateSpline(np.append(-radius_per_bin[0],radius_per_bin),\
                                                np.append(density_in_plane[0],density_in_plane),k=1)
...
print "Desired average cell mass ",gas_mass
...
add_central_cell = 1
if (add_central_cell):
  central_rho = DENSITY_FLOOR #if there is a cavity
...
N_gas = R_gas.shape[0]
print "\nTotal number of mesh-generating points in the model=",N_gas
print "Maximum sampled R: %6.2f\n" % (R_gas.max())
print "Maximum sampled z: %6.2f\n" % (z_gas.max())
...
print "Setting up cartesian coordinates..."
...
#cutting edge of sampling region
#print "Discarding cells outside R_cut..."
#ind= (R_gas< 1.01 * R_cut)
...
if (add_background):
  ...
  if (not load_mesh):
    ...
    print "               maximum R:",R_back.max(), "maximum z:",z_back.max()

  print "   Background stage #1..."
 
  low_res_size = 0.8*(gas_mass/DENSITY_EDGE/np.pi/4.0*3.0)**0.33

  if (maxvolume < meanvolume): maxvolume = meanvolume
  if (low_res_size > R_disk): low_res_size = 0.5 * R_disk
  if (low_res_size < R_disk*0.01): low_res_size = 0.01 * R_disk
  ...
  R_ref = max(R_cut*1.05,R_back.max())
  x, y, z = np.mgrid[(-BOX_WIDTH+low_res_size)/2:(BOX_WIDTH-low_res_size)/2: 2.0*low_res_size,
                     ...
                     -1.5*z_max: 1.5 * z_max : 2.0 *low_res_size]
  ...
  x, y, z = np.mgrid[(-BOX_WIDTH/2+1.5*low_res_size):(BOX_WIDTH/2-1.5*low_res_size): 2.0*low_res_size,
                     ...
                     (-1.5 *z_max + low_res_size):(1.5 * z_max - low_res_size): 2.0 *low_res_size]
  ...
  if (BOX_HEIGHT/2 > z_background.max()):
    ...
    if (DENSITY_BACKGROUND < DENSITY_FLOOR): DENSITY_BACKGROUND = DENSITY_FLOOR
    R_ref = R_back.max()
    low_res_size = 1.0 * (gas_mass/DENSITY_EDGE/np.pi/4.0*3.0)**0.33
    ...
    x, y, z = np.mgrid[-0.5*BOX_WIDTH:0.5*BOX_WIDTH: 2.0*low_res_size,
                       ...
                       -3*z_max: 3 * z_max : 2.0 *low_res_size]
    x_back, y_back, z_back = x.flatten(), y.flatten(), z.flatten()
    x, y, z = np.mgrid[-0.5*BOX_WIDTH+low_res_size:0.5*BOX_WIDTH: 2.0*low_res_size,
                       ...
                       (-3*z_max + low_res_size):3 * z_max: 2.0 *low_res_size]
    x_back, y_back, z_back = np.append(x_back,x.flatten()),np.append(y_back,y.flatten()),np.append(z_back,z.flatten())
  ...
  if ((BOX_HEIGHT/2 > z_background.max()) & (x_back.shape[0] >0)):
    ...
    ind = ((np.abs(z_back) > z_background.max()) & (np.abs(z_back)< 2.4 * z_background.max())\
           & (R_back < 1.8*R_ref) & (R_back < 0.5*BOX_WIDTH)) \
  ...
  if ((BOX_HEIGHT/2 > z_background.max()) & (x_back.shape[0] >0)): 
    ...
    ind = ((np.abs(z_back) > z_background.max()) & (np.abs(z_back) < BOX_WIDTH/2)) \
           | ((R_back > R_ref) & (R_back < 0.5*BOX_WIDTH))
    x_back, y_back, z_back = x_back[ind], y_back[ind], z_back[ind]
    R_back = R_back[ind]
  ...
  vx_gas = np.append(vx_gas,vx_background[ind])
  vy_gas = np.append(vy_gas,vy_background[ind])
  vz_gas = np.append(vz_gas,vz_background[ind])
  mass = np.append(mass,np.repeat(gas_mass*1e-10,N_BACKGROUND))
  dens_gas = np.append(dens_gas,dens_background[ind])
  utherm = np.append(utherm,press_background[ind]/dens_background[ind]/(GAMMA-1))
...
print meanvolume, maxvolume

##########################################################
print "Size of position array...",x_gas.shape,y_gas.shape,z_gas.shape
print "Size of velocity array...",vx_gas.shape,vy_gas.shape,vz_gas.shape
print "Size of density array...",dens_gas.shape
print "Size of energy array...",utherm.shape
print "Size of ID array...",ids.shape
...
if ((meanvolume > 0.1) & (meanvolume <=10)):
...
f = open('param_ICs.txt','w')
f.writelines("%----  Relevant files \n")
f.writelines("InitCondFile        ../disk %-with-grid \n")
f.writelines("OutputDir           ./\n")
f.writelines("EnergyFile          energy.txt\n")
f.writelines("InfoFile            info.txt\n")
f.writelines("TimingsFile         timings.txt\n")
f.writelines("CpuFile             cpu.txt\n")
f.writelines("RestartFile         restart\n")
f.writelines("SnapshotFileBase    snap\n")
f.writelines("OutputListFilename  outputs_fine.txt\n")
f.writelines("%TreecoolFile        ./TREECOOL\n")
f.writelines("\n")
f.writelines("%---- File formats\n")
f.writelines("ICFormat           3 \n")
f.writelines("SnapFormat         3 \n")
f.writelines("\n")
f.writelines("%---- CPU-time limits\n")
f.writelines("TimeLimitCPU              700000% in seconds\n")
f.writelines("CpuTimeBetRestartFile     7000   % in seconds\n")
f.writelines("ResubmitOn        0\n")
f.writelines("ResubmitCommand   my-scriptfile \n")
f.writelines("\n")
f.writelines("%----- Memory alloction\n")
f.writelines("MaxMemSize        4000      % sets maximum memory use in MByte\n")
f.writelines("BufferSize        100       % in MByte\n")
f.writelines("BufferSizeGravity 100\n")
f.writelines("%---- Caracteristics of run\n")
f.writelines("TimeBegin               0.0        % Begin of the simulation\n")
f.writelines("TimeMax              1000.0        % End of the simulation\n")
f.writelines("\n")
f.writelines("%---- Basic code options that set the type of simulation\n")
f.writelines("ComovingIntegrationOn    0\n")
f.writelines("PeriodicBoundariesOn     1 \n")
f.writelines("CoolingOn                1\n")
f.writelines("StarformationOn          0\n")
f.writelines("\n")
f.writelines("%---- Cosmological parameters\n")
f.writelines("Omega0                0\n")
f.writelines("OmegaLambda           0\n")
f.writelines("OmegaBaryon           0\n")
f.writelines("HubbleParam           0.0\n")
f.writelines("BoxSize             %6.1f\n" % BOX_HEIGHT)
f.writelines("\n")
f.writelines("%---- Output frequency and output paramaters\n")
f.writelines("OutputListOn              0\n")
f.writelines("TimeBetSnapshot           1.0\n")
f.writelines("TimeOfFirstSnapshot        0.0\n")
f.writelines("TimeBetStatistics          2.0\n")
f.writelines("NumFilesPerSnapshot        1\n")
f.writelines("NumFilesWrittenInParallel  1\n")
f.writelines("\n")
f.writelines("%---- Accuracy of time integration\n")
f.writelines("TypeOfTimestepCriterion  0\n")
f.writelines("ErrTolIntAccuracy        0.03\n")
f.writelines("CourantFac               0.4 %0.2\n")
f.writelines("MaxRMSDisplacementFac    0.125\n")
f.writelines("MaxSizeTimestep          1.0\n")
f.writelines("MinSizeTimestep          1.0e-6\n")
f.writelines("\n")
f.writelines("\n")
f.writelines("%---- Treatment of empty space and temperature limits\n")
f.writelines("InitGasTemp            0.0\n")
f.writelines("MinGasTemp             0.0\n")
f.writelines("MinimumDensityOnStartUp              %8.6e\n" % DENSITY_FLOOR)
f.writelines("LimitUBelowThisDensity               %8.6e\n" % (DENSITY_EDGE/100.0))
f.writelines("LimitUBelowCertainDensityToThisValue %8.6e\n" % UTHERM_FLOOR)
f.writelines("MinEgySpec             %12.9f\n" % UTHERM_FLOOR)
f.writelines("\n")
f.writelines("%---- Tree algorithm, force accuracy, domain update frequency\n")
f.writelines("TypeOfOpeningCriterion       1\n")
f.writelines("ErrTolTheta                  0.7\n")
f.writelines("ErrTolForceAcc               0.002\n")
f.writelines("MultipleDomains              16\n")
f.writelines("MaxTimeBinsWithoutDomainDecomposition 4\n")
f.writelines("TopNodeFactor                4\n")
f.writelines("\n")
f.writelines("%---- Initial density estimate\n")
f.writelines("DesNumNgb              64 \n")
f.writelines("MaxNumNgbDeviation     1 \n")
f.writelines("\n")
f.writelines("%---- System of units\n")
f.writelines("UnitLength_in_cm         1.49598e+13  % AU\n")
f.writelines("UnitMass_in_g            1.98892e+33  % solar mass\n")
f.writelines("UnitVelocity_in_cm_per_s 4.74057581e+05 % AU per year\n")
f.writelines("GravityConstantInternal  39.4751488\n")
f.writelines("\n")
f.writelines("%---- Gravitational softening lengths\n")
f.writelines("MinGasHsmlFractional 0.1  \n")
f.writelines("SofteningGas      %3.1e\n" % (3*R_min/2.8))
f.writelines("SofteningHalo      0 \n")
f.writelines("SofteningDisk      0\n")
f.writelines("SofteningBulge     0\n")
f.writelines("SofteningStars    %4.1f\n" % (star_soft/2.8))
f.writelines("SofteningBndry     0\n")
f.writelines("SofteningGasMaxPhys       0 \n")
f.writelines("SofteningHaloMaxPhys      0 \n")
f.writelines("SofteningDiskMaxPhys      0 \n")
f.writelines("SofteningBulgeMaxPhys     0 \n")
f.writelines("SofteningStarsMaxPhys     0 \n")
f.writelines("SofteningBndryMaxPhys     0 \n")
f.writelines("GasSoftFactor      3.5 \n")
f.writelines("\n")
f.writelines("\n")
f.writelines("\n")
f.writelines("%non-common\n")
f.writelines("%----- Mesh regularization options\n")
f.writelines("%CellShapingFactor  0.2             % sets threshold for CM-displacement\n")
f.writelines("CellShapingSpeed    0.5            % sets speed of CM correction\n")
f.writelines("CellMaxAngleFactor  1.5\n")
if ((meanvolume > 0.1) & (meanvolume <=10)):
  f.writelines( "MeanVolume              %4.2e\n" % (np.ceil(maxvolume)))
if ((meanvolume > 10) & (meanvolume <=100)):
  f.writelines( "MeanVolume              %4.2e\n" % (np.ceil(maxvolume * 0.1) * 10))
if ((meanvolume > 100) & (meanvolume <=1000)):
  f.writelines( "MeanVolume              %4.2e\n" % (np.ceil(maxvolume * 0.01) * 100))
if (meanvolume > 1000):
  f.writelines( "MeanVolume              %4.2e\n" % (np.ceil(maxvolume * 0.001) * 1000))
f.writelines("RefinementCriterion    1\n")
f.writelines("DerefinementCriterion  1\n")
f.writelines("ReferenceGasPartMass    %5.3e\n" % gas_mass)
f.writelines("TargetGasMassFactor    1.0\n")
f.writelines("\n")
f.writelines("IrradiationTempScaling %12.10f\n" %
             (GasTemperature(0.0) / np.sqrt(M_star) / SOLAR_EFF_TEMP))
f.writelines("IrradiationTempProfileIndex 1.0\n")
f.writelines("CircumstellarSinkRadius  0.2\n")
f.writelines("%CircumstellarSinkEfficiency  0.01\n")
f.writelines("%CircumstellarDerefinementDistance  0.1\n")
f.writelines("%CircumstellarSinkDensityThreshold  1.0e-4\n")
f.writelines("\n")
f.writelines("%CentralMass 1.0\n")
f.writelines("%SofteningCentral 2.0\n")
f.writelines("\n")
f.writelines("%Viscosity parameters\n")
f.writelines("%DynamicViscosity 0.0\n")
f.writelines("%BulkViscosity 0.0\n")
f.writelines("\n")
f.writelines("\n")
f.writelines("\n")
f.close
