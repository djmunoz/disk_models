...
s =           float(sys.argv[8])
################### Box size
Box =         float(sys.argv[9])
################### output ICs filename
ic_filename = sys.argv[10]

#SECONDARY PARAMETERS
##################
star_type = 4
###################
GAMMA       = 1.40000 #adiabatic index
...
if (N_gas < 50000): R_bins = 200
elif (N_gas < 100000): R_bins = 400
elif (N_gas < 200000): R_bins = 800
elif (N_gas < 400000): R_bins   = 1000
else : R_bins = 1200
R_cut    = 6.0 * R_disk
scale_height_cut    = 3.5 #cut in the z direction after a number of scale heights
zeta_bins   = 1200
zeta_max = 12.0
z_max = h_Rd * R_disk * 20
...
R_min = 0.5* np.sqrt(M_disk/N_gas/GasMass(dens_soft))*dens_soft
...
for k in range(0,INTERPOL_BINS):
  ...
  if (k > 0):
    delta_bin = radial_bins[k]-radial_bins[k-1]
    in_bin= np.where(bin_inds == k)
    mass_in_bin = vecGasMass(radial_bins[k]) - vecGasMass(radial_bins[k-1])
  else:
  ...
  if (k < INTERPOL_BINS - 1):
    radius = 0.5*(radial_bins[k+1] + radial_bins[k])
  else:
    radius = R_max
  ...
  def local_pressure(R_vals,z_vals):
    return local_density(R_vals,z_vals)* vecSoundSpeedSquared(R_vals)
  ...
  if (N_in_bin != 0):
    ...
    if(add_background) & (not load_mesh):
      ...
      dens_back_at_R, press_back_at_R = np.zeros(2*z_back_at_R.shape[0]), np.zeros(2*z_back_at_R.shape[0])
      ...
      if (vertical_bins.max() < z_max):
        if (k % 2 == 0):
          R_back_at_R = np.repeat(radius,N_back_in_bin/2)
          z_back_at_R = np.logspace(np.log10(np.abs(z_back_at_R).max()+ Delta_z_at_R),np.log10(min(z_max,5*np.abs(z_back_at_R).max())),N_back_in_bin/2)

          #dens_back_at_R, press_back_at_R = np.repeat(-1.0e-30,2*z_back_at_R.shape[0]), np.repeat(-1.0e-30,2*z_back_at_R.shape[0])
          dens_back_at_R, press_back_at_R = np.zeros(2*z_back_at_R.shape[0]), np.zeros(2*z_back_at_R.shape[0])
          R_back_at_R = np.append(R_back_at_R,R_back_at_R)
          z_back_at_R = np.append(z_back_at_R,-z_back_at_R)  
          dens_back_at_R = local_density(radius,np.abs(z_back_at_R))#/1000
          ...
          max_z_back = np.append(max_z_back,np.abs(z_back_at_R).max())
...
vecEffectiveGasSigma = InterpolatedUnivariateSpline(radius_per_bin,effective_gas_sigma,k=3)
...
print "MASS", quad(GasMass_Integrand,0.0,R_cut)[0]
...
R_gas, z_gas = np.append(R_gas,0.0), np.append(z_gas,0.0)
...
if (add_background):
  ...
  if (not load_mesh):
    ...
    print "               average density :",dens_back.mean(),dens_back.max(),dens_back.min()
  ...
  ind = ((R_back < 1.01* R_ref) & (np.abs(z_back) > vecMaxZBack(R_back)))
  ...
  print "               average density :",DENSITY_BACKGROUND
  
  x_background=np.append(x_background,x_back)
  y_background=np.append(y_background,y_back)
  z_background=np.append(z_background,z_back)
  
  dens_background=np.append(dens_background,np.zeros(x_back.shape[0])+DENSITY_CUTOFF)
  press_background=np.append(press_background,np.zeros(x_back.shape[0])+DENSITY_CUTOFF*SOUNDSPEED_SQUARED_FLOOR)
  ...
  if (BOX_HEIGHT/2 > z_background.max()):
    ...
    dens_background=np.append(dens_background,np.zeros(x_back.shape[0])+DENSITY_BACKGROUND)
    press_background=np.append(press_background,np.zeros(x_back.shape[0])+PRESSURE_BACKGROUND)
