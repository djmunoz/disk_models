...
'''
Reading and writing HDF5 snapshots
'''
        ...
        for parttype in range(0, 5):
            ...
            if (f.keys().__contains__(part_name)):
                ...
                if (f.__getitem__(part_name).__contains__(block_name)):
                    if (first):
                        data = f.__getitem__(
                            part_name).__getitem__(block_name)[:]
                        dim1 += data.shape[0]
                        ret_val = data
                        first = False
                    else:
...
def read_block(filename, block, parttype=-1, no_mass_replicate=False, fill_block="", verbose=False):
    ...
    if (multiple_files):
        ...
        for num in range(0, filenum):
            ...
            if (first):
                data = read_block_single_file(
                    curfilename, block_name, dim2, parttype, no_mass_replicate, fill_block_name, verbose)
                dim1 += data.shape[0]
                ret_val = data
                first = False
            else:
        ...
        ret_val = read_block_single_file(
            curfilename, block_name, dim2, parttype, no_mass_replicate, fill_block_name, verbose)
...
def writeheader(f, header):
    ...
    group_header.attrs.__setitem__(
        "NumPart_Total_HighWord", header.nall_highword)
