...
#version v1.2 slighly modifies version v1.1 to produce automatically
#             a parameter file param_disk.txt that is consistent with the
#             model generated and can be directly used on an AREPO run.
...
#
seed     = 42
R_max    = 5.0 * R_disk # maximum sampling radius
...
radial_bins = np.append(radial_bins,np.linspace(R_max,1.2*R_max,20))
...
plt.loglog(radial_bins,np.sqrt(vecSoundSpeedSquared(radial_bins)),'b.')
plt.show()

plt.loglog(radial_bins,vecGasTemperature(radial_bins),'b.')
plt.show()
...
if (plot_effective_sigma):
  plt.loglog(np.linspace(R_min,R_cut,1000),vecSigmaAnalytic(np.linspace(R_min,R_cut,1000)))
  plt.loglog(np.linspace(R_min,R_cut,1000),vecEffectiveGasSigma(np.linspace(R_min,R_cut,1000)))
...
if (add_background):
  ...
  vz_gas = np.append(vz_gas,vy_background[ind])
